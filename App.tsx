...
function App() {
  ...
  const [shiftTypes, setShiftTypes] = useLocalStorage<Record<string, ShiftType>>('shiftTypes', DEFAULT_SHIFT_TYPES);
  const [shifts, setShifts] = useLocalStorage<Record<string, Shift>>('shifts', {});
  ...
  const handleSaveShift = useCallback((startDate: Date, endDate: Date | null, typeId: string, notes: string) => {
  ...
  const handleDeleteShift = useCallback((date: Date) => {
  ...
  const handleSaveSettings = useCallback((updatedShiftTypes: Record<string, ShiftType>) => {
  ...
  return (
    <div className="min-h-screen text-slate-800 transition-colors duration-300">
      <div className="max-w-7xl mx-auto px-2 sm:px-6 lg:px-8 py-4 sm:py-8">
          ...
          <div className="flex items-center space-x-2 sm:space-x-4 mb-4 sm:mb-0">
             <div className="flex items-baseline space-x-1">
                <div className="relative">
                    <button
                        ref={monthButtonRef}
                        onClick={() => { setMonthSelectorOpen(o => !o); setYearSelectorOpen(false); }}
                        className="text-xl sm:text-2xl font-bold text-slate-900 rounded-md px-2 py-1 hover:bg-slate-200 transition-colors"
                    >
                        {getMonthName(currentDate.getMonth())}
                    </button>
                    {isMonthSelectorOpen && (
                        <div ref={monthSelectorRef} className="absolute left-0 top-full mt-2 z-20 w-64 bg-white rounded-lg shadow-xl border border-slate-200">
                            <div className="grid grid-cols-3 gap-1 p-2">
                                {Array.from({ length: 12 }).map((_, i) => (
                                    <button
                                        key={i}
                                        onClick={() => handleMonthSelect(i)}
                                        className={`p-2 rounded-md text-sm font-medium text-center ${currentDate.getMonth() === i ? 'bg-indigo-600 text-white' : 'text-slate-700 hover:bg-slate-100'}`}
                                    >
                                        {getMonthName(i, 'es-ES')}
                                    </button>
                                ))}
                            </div>
                        </div>
                 ...
                 <div className="relative">
                    <button
                        ref={yearButtonRef}
                        onClick={() => { setYearSelectorOpen(o => !o); setMonthSelectorOpen(false); }}
                        className="text-xl sm:text-2xl font-light text-slate-400 rounded-md px-2 py-1 hover:bg-slate-200 transition-colors"
                    >
                        {currentDate.getFullYear()}
                    </button>
                    {isYearSelectorOpen && (
                        <div ref={yearSelectorRef} className="absolute left-1/2 -translate-x-1/2 top-full mt-2 z-20 w-40 bg-white rounded-lg shadow-xl border border-slate-200">
                                ...
                                {Array.from({ length: 101 }).map((_, i) => {
                                    ...
                                    return (
                                        <button
                                            ...
                                            className={`w-full text-center px-4 py-2 text-sm ${isSelected ? 'bg-indigo-600 text-white' : 'text-slate-700 hover:bg-slate-100'}`}
                                        >
                                            {year}
                                        </button>
                                    );
        ...
        <main className="bg-white shadow-lg rounded-lg">
          <div className="grid grid-cols-7 text-center font-semibold text-sm text-slate-600 border-b border-slate-200">
            {WEEK_DAYS.map((day, i) => 
                <div key={day} className="py-3 px-1">
                    <span className="hidden sm:inline">{day}</span>
                    <span className="sm:hidden">{SHORT_WEEK_DAYS[i]}</span>
                </div>
            )}
          </div>
      ...
      <ShiftModal
        ...
        date={selectedDate}
        existingShift={selectedDate ? shifts[toISODateString(selectedDate)] : undefined}
